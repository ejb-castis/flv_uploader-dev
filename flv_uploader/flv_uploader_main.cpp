...
void begin_publish(std::string const& url, std::string& publish_addr, std::string& upload_id, int& ec)
{
  ec = 0;
  ...
  std::string jsonbody="{ \
\"filename\" : \"baseball.mpg\",\
\"assetId\" : \"baseball.mpg\", \
\"title\" : \"baseball\",\
\"provider\" : \"brokering\",\
\"type\" : \"Video\",\
\"recording\" : \"Y\"\
...
void init_publish(std::string const& publish_addr, std::string const& upload_id, castis::streamer::MediaPublishEsContext& context, std::error_code& ec)
{
...
void publish_es(std::string const& publish_addr, std::string const& upload_id, castis::streamer::MediaPublishEsContext& context, std::error_code& ec)
{
  ...
  std::string path =
      "/publish/es/" + upload_id + "/" + std::to_string(context.media_es_.front().get()->frameNumber);
...
int main() {
  ...
  {
    int ec{0};
    begin_publish(url, publish_addr, upload_id, ec);
    state = init;
  }
  {  
    ...
    for (unsigned index=six; index <= eix; ++index )  {
      ...
      if (state == init && ready_to_send(context)) {
        ...
        std::error_code ec;
        publish_es(publish_addr, upload_id, context, ec);
        std::cout << std::endl;
