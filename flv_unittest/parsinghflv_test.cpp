...
#include "flv_util.cpp"
#include "parsingflv.cpp"
#include "parsingh264.cpp"
#include <array>
#include <string>
#include <iostream>
#include <vector>
...
TEST(FLV, get_aac_header_complex4)
{
  ...
  EXPECT_EQ(36501, sample_rate);
  EXPECT_EQ(9, channel_count);
}
...
TEST(FLV, get_aac_header_complex3)
{
  ...
  EXPECT_TRUE(get_aac_header(aac_seq_buffer, sample_rate, channel_count, aac_seq_buffer_len));
  EXPECT_EQ(48000, sample_rate);
  EXPECT_EQ(2, channel_count);
}
...
TEST(FLV, get_aac_header_complex)
{
  ...
  EXPECT_TRUE(get_aac_header(aac_seq_buffer, sample_rate, channel_count, aac_seq_buffer_len));
  EXPECT_EQ(8, sample_rate);
  EXPECT_EQ(3, channel_count);
}
...
TEST(FLV, replace_nalu_start_code_from_mp4_to_ts)
{
  ...
  buffer_t ts_buffer(ts_nal_size);
  ...
  replace_nalu_strat_code_from_mp4_to_ts(4, mp4_buffer, ts_buffer);

  EXPECT_EQ(mp4_nal_size, mp4_buffer.len_);
  EXPECT_EQ(ts_nal_size, ts_buffer.len_);

  EXPECT_TRUE(std::equal(ts_buffer.ptr_, ts_buffer.ptr_ + ts_buffer.len_, ts_nal));
...
TEST(FLV, write_n)
{
  ...
  EXPECT_EQ(30, written);
  EXPECT_EQ(30, dst_buffer.len_);
...
TEST(FLV, read_nbyte_big_endian_to_little_endian)
{
  ...
  EXPECT_EQ(4, buffer.len_);

  uint32_t value32;
  get_4byte(data4, value32);
  EXPECT_EQ(4261483010, value32);
...
TEST(FLV, read_4byte_big_endian_to_little_endian)
{
  ...
  EXPECT_EQ(4610, value32);

  get_4byte(data3, value32);
  EXPECT_EQ(70146, value32);
...
TEST(FLV, read_n_byte_again_and_again)
{
  ...
  buffer_t buffer(12);
  EXPECT_EQ(12, buffer.max_len_);  
  EXPECT_EQ(0, buffer.len_);  

  read_nbyte(data, data_len, buffer);

  EXPECT_EQ(10, buffer.len_);
  EXPECT_EQ(10, data_len);
  ...
  EXPECT_EQ(12, dst_buffer.max_len_);
  EXPECT_EQ(10, dst_buffer.len_);
...
TEST(FLV, read3_bytes_read3_buffer)
{
  ...
  EXPECT_EQ(0, len_2);
  EXPECT_EQ(3, composition_timestamp_offset.len_);
...
TEST(FLV, read_bytes2)
{
  ...
  buffer_t tbuffer(12);
  EXPECT_EQ(12, tbuffer.max_len_);  
  EXPECT_EQ(0, tbuffer.len_);  

  read_nbyte(buffer, data_len, tbuffer, data_len);

  EXPECT_EQ(10, tbuffer.len_);
  EXPECT_EQ(0, data_len);
...
TEST(FLV, to_hex)
{
  ...
  EXPECT_EQ("00 00 01", str);
  
  str = to_hex(dat2, 3);
  EXPECT_EQ("01 00 00", str);
  ...
  uint32_t v32 = 0xe1;
  str = to_hex(v32);
  EXPECT_EQ("00 00 00 E1", str);

  v32 = 0x100102e1;
  str = to_hex(v32);
  EXPECT_EQ("10 01 02 E1", str);
}
